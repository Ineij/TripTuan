...
interface AppState {
  ...
  /** Order ID from the backend — set after draft is created in Step6 */
  orderId: string | null;
...
export function AppProvider({ children }: { children: ReactNode }) {
  const [scene, setScene] = useState<Scene>('sz');
  ...
  const [orderId, setOrderId] = useState<string | null>(
    () => window.localStorage.getItem('xiaotuan_order_id'),
  );
  ...
  const handleSetOrderId = (id: string | null) => {
    setOrderId(id);
    if (id) window.localStorage.setItem('xiaotuan_order_id', id);
    else window.localStorage.removeItem('xiaotuan_order_id');
  };
  ...
  return (
    <AppCtx.Provider
      value={{
        ...
        paid, setPaid,
        weather, setWeather,
        orderId, setOrderId: handleSetOrderId,
      }}
    >
      {children}
    </AppCtx.Provider>
